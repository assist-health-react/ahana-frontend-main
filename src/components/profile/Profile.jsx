...
const Profile = () => {
  ...
  const { nurseDetails, refreshNurseDetails } = useNurse();
  ...
  useEffect(() => {
    if (nurseDetails) {
      setFormData(prev => ({
        ...
        phoneNumber: nurseDetails.phone || '',
        dateOfBirth: nurseDetails.dob ? new Date(nurseDetails.dob).toISOString().split('T')[0] : '',
        gender: nurseDetails.gender || '',
        profilePic: nurseDetails.profilePic || null
      }));
    }
  }, [nurseDetails]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Updating nurse profile with data:', {
        id: nurseDetails.id,
        formData
      });
      ...
      // Call update nurse API
      const response = await nursesService.updateNurse(nurseDetails.id, updateData);
      console.log('Update response:', response);

      if (response.status === 'success') {
        // Refresh nurse details in context
        await refreshNurseDetails();
