...
const StudentInfirmaryList = ({ isOpen, onClose, student }) => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await getAllInfirmaryRecords({
  ...
  const handleEdit = async (record) => {
    try {
      ...
      toast.error('Failed to fetch record details for editing');
    }
  };

  const handleDelete = async (recordId) => {
    try {
      // No need to call delete API here as it's already done in ViewInfirmaryDetails
  ...
  const handleAddSuccess = async () => {
    ...
    await fetchRecords(); // Ensure records are fetched after successful addition
  };
