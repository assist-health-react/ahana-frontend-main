...
const EditInfirmaryRecord = ({ isOpen, onClose, editData, onSuccess }) => {
  ...
  useEffect(() => {
    if (editData) {
      ...
      // Format the date and time from editData
      const date = editData.date ? new Date(editData.date) : new Date();
      const formattedDate = date.toISOString().split('T')[0];
      
      // If time exists in editData use it, otherwise use current time
      const formattedTime = editData.time || new Date().toLocaleTimeString('en-US', { 
        hour12: false, 
        hour: '2-digit', 
        minute: '2-digit' 
      });

      // Check if complaints is in the predefined list or should be "Others"
      const predefinedComplaints = ["Fever", "Headache", "Stomach Pain", "Injury"];
      const isOtherComplaint = !predefinedComplaints.includes(editData.complaints);

      const formattedData = {
        consentFrom: editData.consentFrom ? editData.consentFrom.charAt(0).toUpperCase() + editData.consentFrom.slice(1) : '',
        consentDate: formattedDate,
        consentTime: formattedTime,
        complaints: isOtherComplaint ? 'Others' : editData.complaints,
        otherComplaint: isOtherComplaint ? editData.complaints : '',
        details: editData.details || '',
        treatment: editData.treatmentGiven || editData.treatment || '',
        tablet: editData.medicineProvided ? {
          value: editData.medicineProvided.inventoryId._id || editData.medicineProvided.inventoryId,
          label: editData.medicineProvided.inventoryId.item_name || '',
          stock: editData.medicineProvided.inventoryId.current_stock || 0
        } : null,
    ...
    const fetchTablets = async () => {
      try {
        ...
        if (editData?.medicineProvided) {
          const selectedTablet = inventoryItems.find(t => 
            t.value === (editData.medicineProvided.inventoryId._id || editData.medicineProvided.inventoryId)
          );
          if (selectedTablet) {
            setFormData(prev => ({
              ...prev,
