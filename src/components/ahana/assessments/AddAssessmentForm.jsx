...
const AddAssessmentForm = ({ isOpen, onClose, student = {}, onSuccess, initialData = null, isEditing = false }) => {
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      setFormData({
        ...
        schoolId: school?._id || initialData.schoolId || '',
  ...
  }, [isEditing, initialData, student, school]);
  ...
  const validateForm = () => {
    ...
    // Validate signatures
    if (!formData.nurseSignature) {
      newErrors.nurseSignature = 'Nurse signature is required';
    }
    if (!formData.doctorSignature) {
      newErrors.doctorSignature = 'Doctor signature is required';
    }
