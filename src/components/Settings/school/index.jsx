...
const SchoolManagement = () => {
  ...
  const handlePincodeChange = async (e) => {
    ...
    if (pincode.length === 6) {
      ...
      try {
        const response = await api.get(`/api/v1/common/pincode/${pincode}`);
        ...
        if (Array.isArray(responseData) && responseData[0]?.Status === 'Success') {
          const postOffices = responseData[0].PostOffice;
          ...
          if (Array.isArray(postOffices) && postOffices.length > 0) {
            ...
            console.log('Region Options Created:', regions);
            ...
            console.error('PostOffice array is empty or invalid');
            setRegionOptions([]);
          }
        } else {
